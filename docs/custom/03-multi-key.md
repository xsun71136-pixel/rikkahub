...
1. **停用状态与开关联动**：Key 被健康停用（无效/额度）时，行内开关同步显示为关闭
   （冷却中不算停用，仅徽标倒计时）；用户手动把开关重新打开会同时清除停用/冷却标记，
   与点徽标恢复等效。说明文案已同步。
2. **导入对话框隐私**：「粘贴导入」不再自动读取系统剪贴板（避免误读输入法剪贴板历史、
   验证码等敏感内容，也避免触发系统"读取剪贴板"提示），打开时输入框默认空白，
   仅用户手动粘贴才有内容。

### 回归验证点
- Key 返回 401/403/402 → 徽标"已停用"+ 开关变灰关闭；点开关或点徽标都能恢复。
- 打开「粘贴导入」→ 输入框为空，系统不弹"rikkahub 读取了你的剪贴板"提示。
